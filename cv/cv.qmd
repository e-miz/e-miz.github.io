...
def print_items(df: pd.DataFrame):
    for i in range(len(df)):
      ...
      print(f"\\begin{{tabularx}}{{\\textwidth}}{{>{{\\RaggedRight}}p{{0.75\\textwidth}}  >{{\\RaggedLeft}}p{{0.21\\textwidth}}}} {desc_frag} & {date_frag} {info_str} \end{{tabularx}} \n")
...
service_df = cv_df.loc[(cv_df["Type"] == "Service")].loc[:, ["Item", "Description", "Start Date", "End Date", "Information"]]
...
\begin{small}they/them/theirs | PhD Candidate | Physics | \href{mailto:emiz@umd.edu}{emiz@umd.edu}\end{small}
\end{center}
```
